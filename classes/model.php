...
class model {
    ...
    public function load($id) {
        ...
        $record = $DB->get_record('aiplacement_coursegen_model', ['id' => $id, 'deleted' => 0]);
        if ($record) {
            $this->id = $record->id;
    ...
    public function save() {
        ...
        if (!$this->validate_unique_name()) {
            throw new \moodle_exception('modelnameexists', 'aiplacement_coursegen');
        }
        ...
        if (empty($this->id)) {
            ...
            $this->id = $DB->insert_record('aiplacement_coursegen_model', $record);
            ...
            $DB->update_record('aiplacement_coursegen_model', $record);
            $this->timemodified = $now;
            $this->usermodified = $USER->id;
        }
    ...
    public function delete() {
        ...
        if (!empty($this->id)) {
            $DB->set_field('aiplacement_coursegen_model', 'deleted', 1, ['id' => $this->id]);
            $this->deleted = 1;
            return true;
        }
        return false;
    ...
    public static function get_all() {
        ...
        return $DB->get_records('aiplacement_coursegen_model', ['deleted' => 0], 'timecreated DESC');
    }
    ...
    public static function get_by_course($courseid) {
        global $DB;
        return $DB->get_record('aiplacement_coursegen_model', ['courseid' => $courseid, 'deleted' => 0]);
    }
}
