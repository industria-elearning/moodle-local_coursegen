...
/**
 * Helper class to manage AI module generation jobs (streaming) stored in aiplacement_coursegen_module_jobs.
 *
...
class module_jobs {
    ...
    public static function save_job(int $courseid, string $jobid, array $data = []): bool {
        ...
        $now = time();
        $record = $DB->get_record('aiplacement_coursegen_module_jobs', ['job_id' => $jobid]);
        ...
        if ($record) {
            $entry->id = $record->id;
            return $DB->update_record('aiplacement_coursegen_module_jobs', $entry);
        } else {
            $entry->timecreated = $now;
            return (bool)$DB->insert_record('aiplacement_coursegen_module_jobs', $entry);
        }
    }
    ...
    public static function update_status(string $jobid, string $status): bool {
        ...
        return $DB->update_record('aiplacement_coursegen_module_jobs', $record);
    }
    ...
    public static function get_by_jobid(string $jobid) {
        global $DB;
        return $DB->get_record('aiplacement_coursegen_module_jobs', ['job_id' => $jobid]);
    }
    ...
    public static function get_latest_for_course(int $courseid, ?int $userid = null) {
        ...
        $sql = "SELECT * FROM {aiplacement_coursegen_module_jobs} ";
