    ...
    public static function execute(int $courseid, int $sectionnum, string $jobid, ?int $beforemod) {
        ...
        try {
            ...
            $aicontext = $DB->get_record_sql(
                'SELECT cc.context_type, m.name FROM {aiplacement_coursegen_course_context} cc
                    LEFT JOIN {aiplacement_coursegen_model} m
                ON cc.model_id = m.id
                    WHERE cc.courseid = ?',
                [$courseid]
            );
            ...
            if (empty($jobid)) {
                return [
                    'ok' => false,
                    'message' => get_string('error_generating_resource', 'aiplacement_coursegen'),
            ...
            $issubclass = is_subclass_of($paramclass, \aiplacement_coursegen\mod_parameters\base_parameters::class);
            ...
            $classpath = '\\aiplacement_coursegen\\mod_settings\\' . $modname . '_settings';
            if (!empty($modsettings) && class_exists($classpath)) {
                if (is_subclass_of($classpath, base_settings::class)) {
                    ...
                    debugging("{$classpath} is not a subclass of \aiplacement_coursegen\mod_settings\base_settings");
                }
            }
            ...
            return [
                'ok' => true,
                'message' => get_string('resource_created', 'aiplacement_coursegen', $modname),
                'data' => [
