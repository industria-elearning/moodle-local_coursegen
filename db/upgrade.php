 ...
 * @return bool
 */
function xmldb_local_datacurso_upgrade($oldversion) {
    global $DB;

    $dbman = $DB->get_manager();
    // Add local_datacurso_model table.
    if ($oldversion < 2025091701) {
        // Define table local_datacurso_model to be created.
        $table = new xmldb_table('local_datacurso_model');

        // Adding fields to table local_datacurso_model.
        ...
        // Adding keys to table local_datacurso_model.
        $table->add_key('primary', XMLDB_KEY_PRIMARY, ['id']);
        $table->add_key('usermodified', XMLDB_KEY_FOREIGN, ['usermodified'], 'user', ['id']);

        // Conditionally launch create table for local_datacurso_model.
    ...
    // Add local_datacurso_course_context table.
    if ($oldversion < 2025091704) {
        // Define table local_datacurso_course_context to be created.
        $table = new xmldb_table('local_datacurso_course_context');

        // Adding fields to table local_datacurso_course_context.
        $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
        ...
        // Adding keys to table local_datacurso_course_context.
        $table->add_key('primary', XMLDB_KEY_PRIMARY, ['id']);
        $table->add_key('courseid', XMLDB_KEY_FOREIGN, ['courseid'], 'course', ['id']);
        $table->add_key('model_id', XMLDB_KEY_FOREIGN, ['model_id'], 'local_datacurso_model', ['id']);
        $table->add_key('usermodified', XMLDB_KEY_FOREIGN, ['usermodified'], 'user', ['id']);

        // Conditionally launch create table for local_datacurso_course_context.
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }
    ...
    if ($oldversion < 2025092001) {
        ...
        // Adding fields to table local_datacurso_course_sessions.
        $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
        $table->add_field('courseid', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, null);
        ...
        // Adding keys to table local_datacurso_course_sessions.
        $table->add_key('primary', XMLDB_KEY_PRIMARY, ['id']);
        $table->add_key('courseid', XMLDB_KEY_FOREIGN, ['courseid'], 'course', ['id']);
        $table->add_key('userid', XMLDB_KEY_FOREIGN, ['userid'], 'user', ['id']);

        // Conditionally launch create table for local_datacurso_course_sessions.
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }
    ...
    // Add local_datacurso_module_jobs table for AI module generation tracking.
    if ($oldversion < 2025092401) {
        // Define table local_datacurso_module_jobs to be created.
        $table = new xmldb_table('local_datacurso_module_jobs');

        // Adding fields to table local_datacurso_module_jobs.
        $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
        $table->add_field('courseid', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, null);
        $table->add_field('userid', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, null);
        ...
        // Adding keys to table local_datacurso_module_jobs.
        $table->add_key('primary', XMLDB_KEY_PRIMARY, ['id']);
        $table->add_key('courseid', XMLDB_KEY_FOREIGN, ['courseid'], 'course', ['id']);
        $table->add_key('userid', XMLDB_KEY_FOREIGN, ['userid'], 'user', ['id']);

        // Conditionally launch create table for local_datacurso_module_jobs.
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }
