    ...
    form.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        if (streamingSection) {
          ...
          const progressIndicator = streamingSection.querySelector("[data-region='local_coursegen/course_streaming/progress']");
          if (progressIndicator) {
            const titleElement = progressIndicator.querySelector("h6, h5");
            const subtitleElement = progressIndicator.querySelector("small");
            if (titleElement) {
              const titleText = await Str.get_string('module_creation_title', 'local_coursegen');
              titleElement.textContent = titleText;
            }
            if (subtitleElement) {
              const subtitleText = await Str.get_string('module_creation_subtitle', 'local_coursegen');
        ...
        if (streamingSection) {
          streamingSection.style.display = "block";
          const eventList = streamingSection.querySelector("[data-region='local_coursegen/course_streaming']");
          if (eventList) {
            const errorDiv = document.createElement("div");
            errorDiv.className = "alert alert-danger mb-2";
            const errorMsg = err.message || await Str.get_string("addactivityai_error", "local_coursegen");
  ...
  const renderWSResult = (wrap, res) => {
    ...
    if (res?.success === false) {
      if (res?.message) {
        ...
        Str.get_string("addactivityai_faildefault", "local_coursegen").then(
          (s) => pushAI(wrap, s)
        );
      }
      return;
    ...
    if (lines.length) {
      ...
      setTimeout(() => {
        const last = wrap.querySelector(".local_coursegen_ai_msg.ai:last-child .bubble");
        if (last) last.textContent = lines.join("\n");
      }, 50);
    } else {
      Str.get_string("addactivityai_done", "local_coursegen").then((s) =>
        pushAI(wrap, s)
      );
    }
  };
