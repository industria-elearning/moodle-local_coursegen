 ...
 *
 * @module     aiplacement_coursegen/add_course_ai_modal
...
} from "aiplacement_coursegen/course_streaming";
import {
  planCourseMessage,
  planCourseExecute,
} from "aiplacement_coursegen/repository/chatbot";
...
export const init = async (params = {}) => {
  try {
    ...
    const [title, bodyHTML, footerHTML] = await Promise.all([
      get_string("addcourseai_modaltitle", "aiplacement_coursegen"),
      Templates.render("aiplacement_coursegen/add_course_ai_modal", {}),
      Templates.render("aiplacement_coursegen/add_course_ai_modal_footer", {}),
    ]);
    ...
    currentModal.getRoot().addClass("aiplacement_coursegen_course_ai_modal");
  ...
  if (acceptBtn) {
    acceptBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        if (!courseId) {
          throw new Error(
            await get_string("error_no_course_id", "aiplacement_coursegen")
          );
        }
        ...
        if (!response.success) {
          throw new Error(
            response.message ||
              (await get_string("error_executing_plan", "aiplacement_coursegen"))
          );
        }
        ...
        if (response.data && response.data.streamingurl && executionContainer) {
          ...
          const html = await Templates.render(
            "aiplacement_coursegen/course_streaming_inline",
            {
        ...
        const message = await get_string("addcourseai_done", "aiplacement_coursegen");
    ...
    chatForm.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        if (!response.success) {
          ...
          const errorMsg =
            response.message ||
            (await get_string("error_processing_request", "aiplacement_coursegen"));
